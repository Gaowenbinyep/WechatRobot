...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def save_model(self, output_dir: Optional[str] = None) -> None:
        ...
        if self.is_fsdp_enabled or self.is_deepspeed_enabled:
            try:
                # ✅ 只在主进程解除共享（否则 ZeRO3 报错）
                if self.accelerator.is_main_process:
                    with torch.no_grad():
                        lm_head = self.model.pretrained_model.base_model.model.lm_head
                        embed_tokens = self.model.pretrained_model.base_model.model.model.embed_tokens

                        if lm_head.weight.data_ptr() == embed_tokens.weight.data_ptr():
                            print("⚠️ 检测到权重共享，正在解除...")
